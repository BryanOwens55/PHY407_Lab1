...
Purpose: To simulate the process of proteins folding and to
         calculate and analyze the energy using Monte Carlo
...
def main(eps, N, T, n):
    ...
    # plt.yticks([])
    plt.tight_layout()
    #plt.savefig('final_protein_T{0:d}_N{1:d}_n{2:d}.pdf'.format(int(10*T), N, n), dpi=dpi)
...
# Run main method using different initial conditions
main(eps, N, T, n)
...
# Creates temperature array
def temp(T_f, T_steps, n):
    T_i = T_f + T_steps - 1
    T_array = np.zeros(n)
...
T_f = 0.5
T_steps = 4
T_array = temp(T_f, T_steps, n)

# Run main code with initial conditions above
main(eps, N, T_array, n)
...
# Energy and standard deviation arrayS
energys = []
energy_std = []

# Run main function then plot output
temps = np.arange(10, 0, -0.5)
for i in temps:
    print(i)
    energy_i = main(eps, N, i, n)
    energys.append(np.mean(energy_i))
    energy_std = np.std(energy_i)
    
plt.errorbar(temps, energys, yerr=energy_std)
